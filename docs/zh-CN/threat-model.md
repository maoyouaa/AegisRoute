...
| Release 自动化权限扩大 | 仓库 workflow 默认只读；Release Please 与镜像发布使用相互独立的 job 级权限；只有 `release_created=true` 才允许发布；不使用 PAT 或 GitHub App 凭据 | GitHub 将创建 PR 与批准 PR 合并为一个可变仓库设置。被攻陷且具有 pull-request 写权限的 Release job 可能滥用该能力，仓库 CI 也无法证明远端实时设置 |
